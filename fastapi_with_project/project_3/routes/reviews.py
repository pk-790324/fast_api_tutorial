...
router=APIRouter(
    prefix="/review",  # means every endpoints starts with /review
    tags=["review"]    # this groups the endpoints in the automatic swagger documentation
)
...
def list_review(
    ...
    limit:int=Query(10,ge=1,le=50,description="max review to return"),
...
def get_average_rating(play_name:str,session:Session=Depends(get_session)):
    ...
    if total_reviews==0:
        raise HTTPException(
            status_code=404,
            detail=f"No Review found for {play_name}"
        )
    return {
        "play_name":play_name,
...
def delete_review(review_id:int,session:Session=Depends(get_session)):
    ...
    session.commit()
    return {"message":"{review_id} is deleted"}
